...
def get_mangrove_vegetation_analysis():
    ...
    try:
        ...
        # Fetch Sentinel-2 images over past 8 years, max 5 with lowest cloud
        while len(images_found) < 5 and current_start > start_date:
            ...
            for i in range(count):
                ...
                # Avoid duplicates based on RGB base64 string
        ...
        # Calculate vegetation loss based on mean NDVI difference between first and last images
        ndvi_loss = ndvi_mean_values[0] - ndvi_mean_values[-1]
        vegetation_loss_percent = (ndvi_loss / ndvi_mean_values[0]) * 100 if ndvi_mean_values[0] != 0 else 0
        
        # Detect poisoning if loss exceeds 5%
        poisoning_detected = vegetation_loss_percent > 5
